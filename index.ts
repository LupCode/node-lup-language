...
export let DEFAULT_LANGUAGES: Array<string> = ["en"];
...
let LANGUAGES: any = {}; // { translationsDir: [] }
let DICTONARY: any = {}; // { translationsDir: {lang: {key: translation} } }
 ...
 */
export const reloadTranslations = async function(translationsDir: string = DEFAULT_TRANSLATIONS_DIR){
...
export function LanguageRouter(options: any = {
    ...
    let langs: Array<string> = []; // later converted to Set 'languages'
    ...
    if(loadTranslations){
        reloadTranslations(translationsDir).then(function(langs: any){
    ...
    return function(req: any, res: any, next: any){
        ...
        if(!lang && useHttp && req.headers){
            ...
            if(langs){
                langs = langs.split(/,|;/g).map(function(v: string){ return v.trim(); }).filter(function(v: string){ return v.length > 0 && !v.startsWith("q="); });
                for(let i=0; i < langs.length; i++)
